...
<?php
		...
		try {
    		$a = $fb->api(array('method'=>'admin_getAppProperties', 'properties'=>'connect_url'));
		} catch (Exception $e) {
...
function fp_is_fan($pageid='0') {
	...
	if ($fb->api(array('method'=>'pages.isfan','page_id'=>$pageid,'uid'=>$fbuid))) {
		return true;
	} else {
		return false;
	}
...
function fp_comm_fill_in_fields($comment_post_ID) {
	...
	if ($fbuid) {
		$user_details = $fb->api(array('method'=>'users.getinfo', 'uids'=>$fbuid, 'fields'=>'name, profile_url'));
		...
		$email = $fb->api(array('method'=>'fql.query', 'query'=>$query));
		if (is_array($email)) {
			$email = $email[0]['email'];
			$_POST['email'] = $email; 
		}
...
function fp_login_ajax_update_fbuid() {
	...
	if (!$disconnect) {
		...
		$ret = $fb->api(array('method'=>'connect.registerUsers','accounts'=>json_encode($aa)));
		if (empty($ret)) { 
			...
			$user_details = $fb->api(array('method'=>'users.getinfo','uids'=>$fbuid, 'fields'=>'email_hashes'));
		...
		$ret = $fb->api(array('method'=>'connect.unregisterUsers', 'email_hashes'=>$hash));
...
// computes facebook's email hash thingy. See http://developers.facebook.com/docs/reference/rest/connect.registerUsers
